...
class HardWork {
  ...
  // do() 이외의 메서드는 수정하지마세요
  get result() {
    return this._result; // -> 초기값 0이 계속 리턴, 클릭 시 연산된 값 리턴
  }
  _initTasks() {
    const count = 30000;
    const tasks = new Array(count); // -> 30000개의 빈 arr

    for (let i = 0; i < count; i++) {
      tasks[i] = this._createTask(Math.floor(Math.random() * 3) + 1);
    }
    return tasks; // -> for 문을 돌려서 만든 30000개의 task가 있는 arr
  }
